...
int main(int argc, char *argv[]) {
  ...
  auto sunMaterial =
      diffuseLight::generateFromImage(image::SUN_TEXTURE, color(10, 10, 10));

  world->add(
      make_shared<sphere>(point3(-8, 0, 0.2), 2, earthMaterial, "Earth"));
  world->add(
      make_shared<sphere>(point3(0, 0, -17.5), 10.0, sunMaterial, "Sun"));
