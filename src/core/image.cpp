...
image::image()
    : m_width(0), m_height(0),
      m_dominantColor(randomDouble(0, 1), randomDouble(0, 1),
                      randomDouble(0, 1)) {}
...
image::image(const char *filename) {
  int bytesPerPixel = 3;
  uint8_t *raw_data =
      stbi_load(filename, &m_width, &m_height, &bytesPerPixel, bytesPerPixel);

  if (!raw_data) {
    std::cerr << "\033[0;31mCould not load texture from file '" << filename
              << "'\033[0m" << std::endl;
#ifdef __APPLE__
    std::cerr << "\tMake sure you are running the executive from its directory."
              << std::endl;
#endif
    m_width = 0;
    m_height = 0;
  ...
  if (bytesPerPixel != TEXTURE_BYTES_PER_PIXEL) {
    std::cerr << "\033[0;31mInvalid bit-depth of the texture '" << filename
              << "'\033[0m" << std::endl;
    m_width = 0;
    m_height = 0;
...
void image::scaleImage(std::shared_ptr<image> destinationImage,
  ...
  float scaleHorizontal =
      static_cast<float>(width()) / destinationImage->width();
  float scaleVertical =
      static_cast<float>(height()) / destinationImage->height();
  ...
  for (int y = 0; y < destinationImage->height(); ++y) {
    for (int x = 0; x < destinationImage->width(); ++x) {
      ...
      destinationImage->at(x, y) =
          topLeft * (1 - wx) * (1 - wy) + topRight * wx * (1 - wy) +
...
std::shared_ptr<image> image::EARTH_DAY_TEXTURE =
    std::make_shared<image>("resources/textures/earthday.jpg");
std::shared_ptr<image> image::EARTH_NIGHT_TEXTURE =
    std::make_shared<image>("resources/textures/earthnight.jpg");
std::shared_ptr<image> image::MARS_TEXTURE =
    std::make_shared<image>("resources/textures/mars.jpg");
std::shared_ptr<image> image::SUN_TEXTURE =
    std::make_shared<image>("resources/textures/sun.jpg");
std::shared_ptr<image> image::MOON_TEXTURE =
    std::make_shared<image>("resources/textures/moon.jpg");
std::shared_ptr<image> image::STARS_TEXTURE =
    std::make_shared<image>("resources/textures/stars.jpg");
std::shared_ptr<image> image::CERES_TEXTURE =
    std::make_shared<image>("resources/textures/ceres.jpg");
std::shared_ptr<image> image::CLOUDY_VENUS_TEXTURE =
    std::make_shared<image>("resources/textures/cloudyvenus.jpg");
std::shared_ptr<image> image::ERIS_TEXTURE =
    std::make_shared<image>("resources/textures/eris.jpg");
std::shared_ptr<image> image::HAUMEA_TEXTURE =
    std::make_shared<image>("resources/textures/haumea.jpg");
std::shared_ptr<image> image::JUPITER_TEXTURE =
    std::make_shared<image>("resources/textures/jupiter.jpg");
std::shared_ptr<image> image::MAKE_TEXTURE =
    std::make_shared<image>("resources/textures/makemake.jpg");
std::shared_ptr<image> image::MERCURY_TEXTURE =
    std::make_shared<image>("resources/textures/mercury.jpg");
std::shared_ptr<image> image::NEPTUNE_TEXTURE =
    std::make_shared<image>("resources/textures/neptune.jpg");
std::shared_ptr<image> image::SATURN_TEXTURE =
    std::make_shared<image>("resources/textures/saturn.jpg");
std::shared_ptr<image> image::URANUS_TEXTURE =
    std::make_shared<image>("resources/textures/uranus.jpg");
std::shared_ptr<image> image::VENUS_TEXTURE =
    std::make_shared<image>("resources/textures/venus.jpg");
