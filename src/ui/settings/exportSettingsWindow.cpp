...
exportSettingsWindow::exportSettingsWindow(std::shared_ptr<camera> cam)
    : m_camera(cam) {}
...
exportSettings exportSettingsWindow::render(RENDER_MODE &mode) {
  ...
  if (ImGui::BeginCombo("Format",
                        (" " + FILE_EXTENSIONS[settings.extension]).c_str())) {
  ...
  if (ImGui::Button("Choose Destination")) {
    FILE *f = popen(
        "zenity  --file-selection --directory --title=\"Choose a directory\"",
        "r");
  ...
  if (ImGui::IsItemHovered() && !ImGui::IsItemActive()) {
    ImGui::BeginTooltip();
    ImGui::TextUnformatted("Maximum number of bounces of every ray.\nUse large "
                           "values only if your objects are very "
                           "close to each other.");
    ImGui::EndTooltip();
  }
  ...
  if (mode == EDIT) {
    ...
    if (ImGui::IsItemHovered() && !ImGui::IsItemActive()) {
      ...
      ImGui::TextUnformatted(
          "The program might run slower after running preview.");
      ImGui::EndTooltip();
    }
  } else {
  ...
  if (ImGui::IsItemHovered() && !ImGui::IsItemActive()) {
    ImGui::BeginTooltip();
    ImGui::TextUnformatted(
        "The program might run slower after starting the export.");
    ImGui::EndTooltip();
  }
